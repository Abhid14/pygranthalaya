...
def createDB():
    ...
    libcur.execute("""CREATE TABLE circulation(UID INT PRIMARY KEY NOT NULL,ACCNO INT UNIQUE NOT NULL,TITLE TEXT NOT NULL,NAME TEXT NOT NULL,CLASS TEXT NOT NULL);""")
    libdb.commit()
    libdb.close()
def createJSON(userData, instData):
    ...
    abtdataD = {'firstName': firstName, 'lastName': lastName, 'userName': userName, 'userEmail': userEmail, 'phoneNumber': phoneNumber,
                'userPassword': userPassword, 'instName': instName, 'instAddress': instAddress, 'instCity': instCity, 'instPIN': instPIN, 'instEmail': instEmail}
    with open(r"{}\\resources\appdata\about.json".format(os.getcwd()), "w") as abtdataF:
